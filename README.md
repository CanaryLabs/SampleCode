...
│
├── V23/                                    Canary version 23 samples
│   ├── NodeRed/
│   │   ├── ModbusClient/
│   │   └── ModbusClient_StoreAndForward/
│   ├── ParquetExport/                      .NET Parquet export sample
│   ├── PowerShell/                         CSV export scripts
│   ├── Python/                             ExportToCSV.py
│   ├── R/                                  ExportToCSV.R
│   ├── SenderNETClient/                    Store & Forward .NET client
│   ├── SenderWebApi/                       Store & Forward Web API (JS)
│   ├── ViewsNETClient/                     Views .NET client
│   └── ViewsWebApi/                        Views Web API (JS)
│
├── V24/                                    Canary version 24 samples
│   ├── StoreAndForwardNETClient/
│   └── ViewsNETClient/
│
└── V26/                                   Canary version 26 samples
    ├── CustomCalculationFunctionPlugin/   Custom calc function plugin
...
`/V26/ViewsClient/ViewsClient.csproj`.

## Dependencies

The samples depend on two kinds of assemblies:

1. **NuGet packages** (V23/V24 only) These are declared as `<PackageReference>` in the V23/V24 `.csproj` files with pinned versions.\*\*
   ...
   >
   > **V26 samples do not use NuGet** — they reference every dependency (Canary _and_ third-party) directly from `DLLs/`. See the V26 section for the full DLL list per project.
...
### Grpc .proto files and Documentation

The grpc .proto files and documentation are available in `C:\Program Files\Canary\Documentation\gRPC Documentation`
...
**Copy into `DLLs/`** (from `C:\Program Files\Canary\Canary Admin`)
...
**Copy into `DLLs/`:** (from `C:\Program Files\Canary\Calculations\`)

- Canary.Calculations.FunctionPlugin.Base.dll
